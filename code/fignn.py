...
def multihead_attention(queries,
    ...
    if has_residual:
        V_res = tf.layers.dense(queries, num_units, activation=tf.nn.relu)
...
def message_pass(x, emb_size, hidden_size, field_size, step, graph=None):
    ...
    for i in range(field_size):
        state = tf.reshape(
            tf.layers.dense(tf.reshape(x[:, i, :], [-1, emb_size]), hidden_size, activation=None,
                            use_bias=None,
                            name="output_layer%d_field%d" % (step, i)), [-1, hidden_size])
        states_1.append(state)
    states_1 = tf.stack(states_1, axis=2)  # [batch_size, hidden_size, field_size]
    
    if graph is None: # fully-connected graph
        states_1 = tf.transpose(states_1, (0, 2, 1))  # [batch_size, field_size, hidden_size]
        states_2 = tf.tile(tf.reshape(tf.reduce_sum(states_1, axis=1), [-1, 1, hidden_size]), [1, field_size, 1])
        # remove the self-loop
        states_2 -= states_1
    else: # input weighted graph
        states_2 = tf.matmul(states_1, graph)
        states_2 = tf.transpose(states_2, (0, 2, 1))  # [batch_size, field_size, hidden_size]

    states_3 = []
    for i in range(field_size):
        state = tf.reshape(
            tf.layers.dense(tf.reshape(states_2[:, i, :], [-1, hidden_size]), emb_size, activation=None,
                            use_bias=None,
                            name="input_layer%d_field%d" % (step, i)), [-1, emb_size])
        states_3.append(state)
    states_3 = tf.stack(states_3, axis=2)  # [batch_size, emb_size, field_size]
    states_3 = tf.transpose(states_3, (0, 2, 1))  # [batch_size, field_size, emb_size]

    return states_3


def GNN(feature_embedding_input, blocks, n_steps, emb_size, field_size, graph=None):

    gru_cell = tf.keras.layers.GRUCell(emb_size)

    h0 = feature_embedding_input
    state = tf.reshape(h0, [-1, field_size, emb_size])
    ...
    with tf.variable_scope("gnn"):
        for step in range(n_steps):
            ...
            hidden_size = blocks[step]
            x = message_pass(state, emb_size, hidden_size, field_size, step, graph) #[-1, field_size, emb_size]
            (x_new, state_new) = gru_cell(tf.reshape(x, [-1, emb_size]), tf.reshape(state, [-1, emb_size]))
            state_new = tf.reshape(state_new, [-1,  field_size, emb_size])
            # Residual
            state = h0 + state_new
...
class FiGNN():
    def __init__(self, args, feature_size, run_cnt):
        ...
        # self.output_size = args.layers[-1]
        # self.num_layers = args.num_layers
        # self.GNN_step = args.num_layers - 1  # number of GNN steps, i.e., the number of graph attention layers, except the initial embedding layer
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.GNN_step = self.blocks - 1 # exlude the initial embedding layer
            self.y_deep, self.y_deeps = GNN(
                self.y_deep, self.block_shape, self.GNN_step, self.output_size, self.field_size)

            # self.out = self.prediction_layer_concat(self.y_deeps)
    ...
    def readout(self, state, i):
        with tf.variable_scope("readout"):
            ...
            g = tf.layers.dense(state, self.block_shape[i], activation=tf.nn.tanh, use_bias=True, name="weights"+str(i))
    ...
    def prediction_layer_concat(self, states):
        emb = []
        for i, state in enumerate(states):
            g_emb = self.readout(state, i)
            emb.append(g_emb)
        z_size = sum(self.block_shape)
        z = tf.reshape(tf.concat(emb, 1), [-1, z_size])
        with tf.variable_scope("prediction"):
            score = tf.layers.dense(z, 1, activation=None, use_bias=True,
                                        name="score")

        return score

    def prediction_layer_add(self, states):
        for i in range(0, (self.blocks-1)):
            assert self.block_shape[i] == self.block_shape[i+1]
        for i, state in enumerate(states):
            g_emb = self.readout(state, i)
            if i == 0:
                emb = g_emb
            else:
                emb = tf.add(emb, g_emb)
        z = emb
        with tf.variable_scope("prediction"):
            score = tf.layers.dense(z, 1, activation=None, use_bias=True,
                                        name="score")
