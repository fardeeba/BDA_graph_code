'''
Tensorflow implementation of InterHAt described in:
...
def single_attention(values,
    ...
    if num_units is None:
        num_units = values.get_shape().as_list[-1]

    # Linear projections
    V = tf.layers.dense(values, num_units, activation=tf.nn.relu, use_bias=None)
    weights = tf.layers.dense(V, 1, activation=None, use_bias=None) #[batch_size, field_size, 1]
        ...
        with self.graph.as_default():
            ...
            for i in range(self.blocks):
                ...
                for j in range(self.field_size):
                    ...
                    state_all = state * h0 # [batch_size, field_size, hidden_size]
                    state_ = single_attention(values=state_all,
